...
from ..models import BloodBank, BranchManager, Broadcast
from ..service.BroadCastService import BroadCastService


class BroadCastListCtl(Basectl):
    count = 1

    def request_to_form(self, requestForm):
        self.form["broadcastCode"] = requestForm.get("broadcastCode", None)
        self.form["ids"] = requestForm.getlist("ids", None)

    def display(self, request, params={}):
        BroadCastListCtl.count = self.form['pageNo']
        records = self.get_service().search(self.form)
        self.page_list = records['data']
        self.form['lastId'] = Broadcast.objects.last().id
        res = render(request, self.get_template(), {'pageList': self.page_list, 'form': self.form})
        return res

    def next(self, request, params={}):
        BroadCastListCtl.count += 1
        self.form["pageNo"] = BroadCastListCtl.count
